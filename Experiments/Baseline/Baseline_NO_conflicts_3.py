...
class Baseline:
    def __init__(self, place, years):
        ...
        self.comp_space = pd.read_csv(f"C:/thesis_code/Github/Experiments/data/{place}_{years}", usecols=[
                                      'pa_id_1', 'pa_id_2', 'fn_score', 'ln_score', 'age_distance'])
        ...
        self.merged_pa_id.drop(
            columns=['age_distance', 'fn_score', 'ln_score'], inplace=True)
        self.match_values = 0
    ...
    def perfect_scores(self, df):
        conditions = [df['age_distance'].eq(
            0) & df['fn_score'].eq(0) & df['ln_score'].eq(0)]
        values = [1]
    ...
    def precision_recall(self, df):
        ...
        f = open(
            f"C:/thesis_code/Github/Experiments/evaluation_results/Baseline_NO_conflicts_3", "a")
        f.write(
            f"\nBaseline 3 features conflicts RESOLVED, place: {self.place}, time: {self.years} \
