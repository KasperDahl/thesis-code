...
class Baseline:
    ...
    def perfect_scores(self, df):
        conditions = [df['age_distance'].eq(0) & df['fn_score'].eq(
            0) & df['ln_score'].eq(0) & df['bp_score'].eq(0)]
        values = [1]
    ...
    def precision_recall(self, df):
        ...
        f = open(
            f"C:/thesis_code/Github/Experiments/evaluation_results/Baseline_conflicts_4", "a")
        f.write(
            f"\nBaseline 4 features conflicts (fam_n excluded), place: {self.place}, time: {self.years} \
