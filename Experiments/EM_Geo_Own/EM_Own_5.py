...
class ExpectationMaximization:
    ...
    def em_steps(self, iterations=100):
        for i in range(iterations):
            ...
            for i in range(len(self.data)):
                ...
                w = (p_A_M+p_FN_M+p_LN_M+p_FAMN_M+p_BP_M) / \
                    ((p_A_M+p_FN_M+p_LN_M+p_FAMN_M+p_BP_M) +
                     (p_A_U+p_FN_U+p_LN_U+p_FAMN_U+p_BP_U))
                w_vector[i] = w
    ...
    def evaluation_bayes(self, data, results, data_b, path):
        ...
        for i in range(len(data)):
            ...
            prob = (P_A_M + P_F_M + P_L_M + P_FAM_M + P_BP_M) / \
                ((P_A_M + P_F_M + P_L_M + P_FAM_M + P_BP_M) +
                 (P_A_U + P_F_U + P_L_U + P_FAM_U + P_BP_U))
            probabilities.append('%.4f' % (prob))
        data_b['EM probabilities'] = probabilities
        data_b.to_csv(
            f"C:/thesis_code/Github/Experiments/results_after_EM/EM_Own_5/{path}", index=False)
        f = open(
            "C:/thesis_code/Github/Experiments/results_after_EM/EM_Own_5/p_values.txt", "a")
